...
export default function ReelsPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex h-screen items-center justify-center bg-black px-3">
  ...
  if (posts.length === 0) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-3 bg-black px-6 text-center text-white">
        <div className="grid size-16 place-items-center rounded-full border-2 border-white">
  ...
  return (
    <div className="no-scrollbar h-screen snap-y snap-mandatory overflow-y-scroll bg-black">
      {posts.map((post) => (
        <ReelCard
          key={post.id}
          post={post}
